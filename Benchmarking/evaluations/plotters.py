...
def run_plotting(pae = True, pre = True, hae = True):
    ...
    if pae:
        plot_absolute_error(loaded_data)
    if pre:
        plot_relative_error(loaded_data)
    if hae:
        plot_histogram_absolute_error(loaded_data)
...
def plot_absolute_error(results_data):
...
def plot_relative_error(results_data):
    for key, runs in results_data.items():
        wrapper, model = key.split('_', 1)  # Split into wrapper and model
        wrapper = wrapper  # Get actual wrapper name from results
        model = model     # Get actual model name from results
...
def plot_histogram_absolute_error(results_data):
    x_max = 2
    y_max = 2
...
def plot_relative_error_ensemble_size_comparisons(results_data):
    pass
