...
class Default_Wrapper(ABC):
    ...
    def cross_validate(self, X, y, n_splits=10, train_percent=100, subsample_seed=67):
        ...
        Returns:
            mean_pred: Mean predictions, concatenated across folds (one flat array, not a
                list of per-fold arrays).
            var_pred: Variance predictions, concatenated across folds the same way (if
                output_variance == True: returns variance, else: returns None).
            y_true: Ground truth targets, concatenated across folds the same way.
            fold_id: Integer array the same length as mean_pred/var_pred/y_true, giving the
                0-based fold index each row came from (e.g. for grouping into a Tukey plot:
                mean_pred[fold_id == 3] gives just fold 3's predictions). Skipped/failed
                folds simply don't appear in fold_id -- it isn't necessarily a contiguous
                0..n_splits-1 range if any folds failed.
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(kf.split(X)):
            ...
            y_true.append(y_val)
            fold_ids.append(np.full(len(val_idx), fold_idx, dtype=int))
        ...
        fold_id = np.concatenate(fold_ids, axis=0)
        
        return mean_pred, var_pred, y_true, fold_id
